<?php
/**
 * BIALYSTOK BREWING CO — Variedades de Lúpulo
 * Reemplaza: variedades_lupulo.php
 */
...
try {
    $pdo = getPDO();
    $stmt_total = $pdo->prepare("SELECT COUNT(*) FROM variedades_lupulo WHERE nombre LIKE ? OR marca LIKE ?");
    $stmt_total->execute([$like, $like]);
    $total = (int) $stmt_total->fetchColumn();
    $total_paginas = max(1, (int) ceil($total / $por_pagina));

    $stmt = $pdo->prepare("SELECT id, nombre, marca FROM variedades_lupulo WHERE nombre LIKE ? OR marca LIKE ? ORDER BY nombre ASC LIMIT $offset, $por_pagina");
...
<div id="contenido" class="main-content">
  <div class="page-header fade-in">
    ...
    <button class="btn btn-primary" onclick="abrirModal()">+ Nuevo lúpulo</button>
  </div>

  <div class="toolbar fade-in">
    <form action="lupulos" method="GET" style="display:contents">
      ...
      <?php if ($searchTerm): ?><a href="lupulos" class="btn btn-ghost btn-sm">✕ Limpiar</a><?php endif; ?>
    </form>
      ...
      <tbody>
        <?php if ($lupulos): ?>
          ...
          <tr style="cursor:pointer" onclick="selRow(this)">
            <td><input type="radio" name="sel" value="<?= (int)$l['id'] ?>" style="accent-color:var(--amber-400)"></td>
            <td><?= e(ucwords(strtolower($l['nombre']))) ?></td>
            <td style="color:var(--text-muted)"><?= e(ucwords(strtolower($l['marca']))) ?></td>
          </tr>
          <?php endforeach; ?>
        <?php else: ?>
          <tr><td colspan="3" style="text-align:center;padding:2rem;color:var(--text-muted)">
            <?= $searchTerm ? 'Sin resultados para "' . e($searchTerm) . '"' : 'No hay lúpulos registrados.' ?>
          </td></tr>
        <?php endif; ?>
      </tbody>
    </table>
...
<div id="modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:1000;align-items:center;justify-content:center">
  <div class="card" style="width:400px;position:relative">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.25rem">
      <h2 style="margin:0;font-size:1rem">Nuevo lúpulo</h2>
    ...
    <div class="form-group">
      ...
      <input type="text" id="inp-nombre" placeholder="Ej: Cascade" autocomplete="off">
    </div>
    <div class="form-group">
      <label class="form-label">Marca / Origen</label>
      <input type="text" id="inp-marca" placeholder="Ej: Yakima Valley" autocomplete="off">
    </div>
...
function getSelected() {
  const r = document.querySelector('#tabla-lupulos input[type="radio"]:checked');
  return r ? r.value : null;
}

document.getElementById('btnEliminar')?.addEventListener('click', function() {
  const id = getSelected();
  if (!id) { alert('Seleccioná un lúpulo primero.'); return; }
  if (!confirm('¿Eliminar este lúpulo?')) return;
  fetch('eliminar_registro', {
    method: 'POST',
    headers: {'Content-Type': 'application/x-www-form-urlencoded'},
    body: new URLSearchParams({ id, tabla: 'variedades_lupulo', csrf_token: '<?= e(getCsrfToken()) ?>' })
...
function guardar(continuar) {
  ...
  fetch('anadir_registro', {
    method: 'POST',
    headers: {'Content-Type': 'application/x-www-form-urlencoded'},
    body: new URLSearchParams({ tabla: 'variedades_lupulo', nombre, marca, csrf_token: '<?= e(getCsrfToken()) ?>' })
  }).then(r => r.json()).then(d => {
    if (d.success) {
      if (continuar) {
